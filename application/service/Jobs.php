...
class Jobs
{
    ...
    public static function push($jobName, $jobClass, $method = '', $params = [], $jobExt = [])
    {
        ...
        $typeDelay = $jobExtras['delay'] == 0 ? 1 : 0;
        $result   = $queue->push($jobName, $job, $typeDelay);
        if($jobExtras['delay']>0){
            $result = ['job_id'=>$result,'job_queue_nane'=>'enqueue.'.$jobName.'.'.$jobExtras['delay'].'.delayed '];
        }else {
            $result = ['job_id'=>$result,'job_queue_nane'=>$jobName];
        }
        return $result;
    }
}
